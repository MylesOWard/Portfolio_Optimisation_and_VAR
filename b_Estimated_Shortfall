...
# logs are much easier to work with than percentage changes and can be summed over time
...
# calculate threashold for threashold VaR 
VaR_threshold = np.percentile(range_returns.values, alpha*100)

# expected shortfall

# accurate method exponentiate 
lowest_value = (np.exp(range_returns.min()) - 1) * portfolio_value

# less accurate method straight from the log
''' lowest_value = range_returns.min() * portfolio_value '''

# calculate expected shortfall, find greatest 5-day loss and print results 
expected_shortfall = -range_returns[range_returns <= VaR_threshold].mean() * portfolio_value
print("Expected shortfall: "+ expected_shortfall)
print("Worst observed loss: " + (lowest_value))
...
plt.title("Distribution of Portfolio 5-Day Dollar Value")
plt.axvline(-expected_shortfall, color='g', linestyle='dashed', linewidth=2,
            label=f'Expected Shortfall at {confidence_level:.0%} confidence level')
plt.legend()
plt.show()
